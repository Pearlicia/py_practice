...
def create_customer(cname,mobile,phone,email,barcode):
    try:
        cust_obj = models.execute_kw(db, uid, password, 'res.partner', 'search_read',
          [[['name', '=', cname]]], {'fields': ['id', 'name', 'phone','mobile','email','barcode']})
        # cust_obj = models.execute_kw(db, uid, password, 'res.partner', 'search_read',
        ...
        if not cust_obj:
            ...
            p_n = models.execute_kw(db, uid, password, 'res.partner', 'create', \
                    [{ 'name': cname,'phone':phone,'mobile':mobile,'customer':False,'supplier':True,'email':email,'barcode':barcode }])
...
with open(WBFILE, 'rb') as csvfile:
    ...
    for row in line:
        try:
            ...
            assert barcode,"no barcode created"
